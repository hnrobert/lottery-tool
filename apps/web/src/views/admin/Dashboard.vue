<template>
  <PageTitle :title="'Dashboard'" :sub-title="'Welcome to the admin dashboard'" />
  <!-- 顶部数据卡片 -->
  <div class="grid md:grid-cols-4 grid-cols-2 gap-4 py-4">
    <NumberCard
      :title="cardLabels.activities"
      :value="dashboardStats.total_activities"
      :icon="Users"
    />
    <NumberCard
      :title="cardLabels.lotteryCodes"
      :value="dashboardStats.total_lottery_codes"
      :icon="ShoppingCart"
    />
    <NumberCard :title="cardLabels.users" :value="dashboardStats.total_users" :icon="Users" />
    <NumberCard
      :title="cardLabels.records"
      :value="dashboardStats.total_lottery_records"
      :icon="Package"
    />
  </div>
  ...
  <div class="grid lg:grid-cols-2 grid-cols-1 gap-8 py-4">
    ...
    <div class="space-y-4">
      <div class="flex items-center justify-between">
        <h3 class="scroll-m-20 text-xl font-semibold tracking-tight">进行中的活动</h3>
    ...
    <div class="space-y-4">
      <div class="flex items-center justify-between">
        <h3 class="scroll-m-20 text-xl font-semibold tracking-tight">最近创建的活动</h3>
...
const fetchAllData = async () => {
  loading.value = true
  try {
    await Promise.all([fetchDashboardStats(), fetchActiveActivities(), fetchRecentActivities()])
  } finally {
    loading.value = false
  }
}
