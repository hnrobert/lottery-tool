...
describe('抽奖码生成器测试', () => {
  ...
  describe('validateLotteryCodeFormat', () => {
    ...
    it('应该验证8位数字格式', () => {
      expect(lotteryCodeGenerator.validateLotteryCodeFormat('12345678', '8_digit_number')).toBe(
        true,
      )
      expect(lotteryCodeGenerator.validateLotteryCodeFormat('1234567', '8_digit_number')).toBe(
        false,
      )
      expect(lotteryCodeGenerator.validateLotteryCodeFormat('123456789', '8_digit_number')).toBe(
        false,
      )
      expect(lotteryCodeGenerator.validateLotteryCodeFormat('1234567a', '8_digit_number')).toBe(
        false,
      )
    })
    ...
    it('应该验证12位数字格式', () => {
      expect(
        lotteryCodeGenerator.validateLotteryCodeFormat('123456789012', '12_digit_number'),
      ).toBe(true)
      expect(lotteryCodeGenerator.validateLotteryCodeFormat('12345678901', '12_digit_number')).toBe(
        false,
      )
      expect(
        lotteryCodeGenerator.validateLotteryCodeFormat('1234567890123', '12_digit_number'),
      ).toBe(false)
      expect(
        lotteryCodeGenerator.validateLotteryCodeFormat('12345678901a', '12_digit_number'),
      ).toBe(false)
    })

    it('应该验证8位数字字母混合格式', () => {
      expect(
        lotteryCodeGenerator.validateLotteryCodeFormat('12345678', '8_digit_alphanumeric'),
      ).toBe(true)
      expect(
        lotteryCodeGenerator.validateLotteryCodeFormat('1234567a', '8_digit_alphanumeric'),
      ).toBe(true)
      expect(
        lotteryCodeGenerator.validateLotteryCodeFormat('1234567', '8_digit_alphanumeric'),
      ).toBe(false)
      expect(
        lotteryCodeGenerator.validateLotteryCodeFormat('123456789', '8_digit_alphanumeric'),
      ).toBe(false)
      expect(
        lotteryCodeGenerator.validateLotteryCodeFormat('1234567A', '8_digit_alphanumeric'),
      ).toBe(false)
    ...
    it('应该验证12位数字字母混合格式', () => {
      expect(
        lotteryCodeGenerator.validateLotteryCodeFormat('123456789012', '12_digit_alphanumeric'),
      ).toBe(true)
      expect(
        lotteryCodeGenerator.validateLotteryCodeFormat('12345678901a', '12_digit_alphanumeric'),
      ).toBe(true)
      expect(
        lotteryCodeGenerator.validateLotteryCodeFormat('12345678901A', '12_digit_alphanumeric'),
      ).toBe(true)
      expect(
        lotteryCodeGenerator.validateLotteryCodeFormat('12345678901', '12_digit_alphanumeric'),
      ).toBe(false)
      expect(
        lotteryCodeGenerator.validateLotteryCodeFormat('1234567890123', '12_digit_alphanumeric'),
      ).toBe(false)
    })
  ...
  describe('getFormatDescription', () => {
    it('应该返回格式描述', () => {
      ...
      expect(lotteryCodeGenerator.getFormatDescription('8_digit_alphanumeric')).toBe(
        '8位数字+小写字母',
      )
      expect(lotteryCodeGenerator.getFormatDescription('12_digit_alphanumeric')).toBe(
        '12位数字+字母',
      )
    })
