...
const tests: Array<() => Promise<void>> = [
  ...
  () =>
    test('批量创建抽奖码', async () => {
      const response = await axios.post(
        `${API_BASE}/admin/activities/${testActivity.id}/lottery-codes/batch`,
        {
          count: 5,
        },
        {
  ...
  () =>
    test('获取抽奖码列表', async () => {
      const response = await axios.get(
        `${API_BASE}/admin/activities/${testActivity.id}/lottery-codes`,
        {
          headers: { Authorization: `Bearer ${authToken}` },
        },
      )
  ...
  () =>
    test('获取Webhook信息', async () => {
      const response = await axios.get(
        `${API_BASE}/admin/activities/${testActivity.id}/webhook-info`,
        {
          headers: { Authorization: `Bearer ${authToken}` },
        },
      )
  ...
  () =>
    test('清理测试数据', async () => {
      if (testActivity && testActivity.id) {
        // 先将活动状态改为draft才能删除
        await axios.put(
          `${API_BASE}/admin/activities/${testActivity.id}`,
          {
            status: 'draft',
          },
          {
  ...
  () =>
    test('验证抽奖码', async () => {
      ...
      const response = await axios.get(
        `${API_BASE}/lottery-codes/validate/${testLotteryCode.code}?activity_id=${testActivity.id}`,
      )
  ...
  () =>
    test('查询抽奖码信息', async () => {
      ...
      const response = await axios.get(
        `${API_BASE}/lottery-codes/query?code=${testLotteryCode.code}&activity_id=${testActivity.id}`,
      )
...
const checkServiceOnline = async (): Promise<void> => {
  ...
  try {
    ...
    console.log(
      '   请先启动服务（pnpm dev / pnpm start），并通过 POST /auth/register 注册首位超级管理员',
    )
    process.exit(1)
  }
}
