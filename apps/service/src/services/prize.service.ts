...
export async function createPrize(
  ...
  if (!check.isValid) {
    throw new Error(
      `添加此奖品后，活动概率总和将超过1（当前总和：${check.totalProbability.toFixed(4)}）`,
    )
  }
...
export async function updatePrize(
  ...
  if (
    ...
    String(updates.probability) !== String(prize.probability)
  ) {
    ...
    if (!check.isValid) {
      throw new Error(
        `修改此奖品概率后，活动概率总和将超过1（当前总和：${check.totalProbability.toFixed(4)}）`,
      )
    }
  }
