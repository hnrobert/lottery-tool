 ...
 * 抽奖码格式映射
 */
export const CODE_FORMATS: Record<string, CodeFormatConfig> = {
  '4_digit_number': {
    length: 4,
    charset: '0123456789'
  },
 ...
 * @returns 生成的抽奖码
 */
export function generateLotteryCode(format: string): string {
 ...
 * @param format - 抽奖码格式
 * @param count - 生成数量
 * @param existingCodes - 已存在的抽奖码列表
 * @returns 生成的抽奖码数组
 */
export function generateBatchLotteryCodes(format: string, count: number, existingCodes: string[] = []): string[] {
  if (count < 0) {
 ...
 * @returns 是否符合格式
 */
export function validateLotteryCodeFormat(code: string, format: string): boolean {
  const config = CODE_FORMATS[format];
  if (!config) {
    throw new Error('不支持的抽奖码格式');
  }
 ...
 * @returns 格式配置对象数组
 */
export function getSupportedFormats(): Array<{ format: string; description: string; example: string }> {
